<template>
  <div class="goods_list">
    <van-search v-model="value"
                shape="round"
                background="#e93323"
                placeholder="搜索商品信息"
                @search="onSearch">
    </van-search>

    <div class="tabbar">
      <div :class="{red:this.$route.query.title}"
           @click="$router.push('/classify')">{{this.$route.query.title?this.$route.query.title:'默认'}}</div>
      <div @click="changenum">
        <span>价格</span>
        <i :class="[{active:num==1},'fa','fa-caret-up']"></i>
        <i :class="[{active:num==2},'fa','fa-caret-down']"></i>
      </div>
      <div @click="changenum2">
        <span>销量</span>
        <i :class="[{active:num2==1},'fa','fa-caret-up']"></i>
        <i :class="[{active:num2==2},'fa','fa-caret-down']"></i>
      </div>
      <div>
        <span>新品</span>
      </div>
    </div>
    <no-search v-if="result.length==0"></no-search>
    <div class="productlist">
      <router-link class="pro"
                   tag="div"
                   :to="`/detail/${i.id}`"
                   v-for="(i,index) in result"
                   :key="index">
        <img :src="i.image">
        <h2>{{i.store_name}}</h2>
        <div class="price">
          <i>￥</i>
          <h1>{{i.price}}</h1>
        </div>
        <div class="vipprice">
          <div class="left">
            <i>￥</i>
            <h1>{{i.vip_price}}</h1>
            <img src="data:image/png;base64,"
                 alt="">
          </div>
          <span>
            已售{{i.sales}}件
          </span>
        </div>
      </router-link>
    </div>
  </div>
</template>

<script>
import NoSearch from '../components/NoSearch.vue';
export default {
  components: { NoSearch },
  data () {
    return {
      value: '',
      payload: '',
      num: 0,
      num2: 0,
      priceOrder: '',
      salesOrder: '',
      result: []
    }
  },
  created () {
    this.axios.get(`/products?keyword=${this.$route.query.title}`).then(result => {
      this.result = result.data.data
    })
  },

  beforeRouteEnter (to, from, next) {
    if (from.name == "Detail") {
      next(vm => {
        if (vm.payload) {
          vm.axios.get(`/products?keyword=${vm.payload}`).then(result => {
            vm.result = result.data.data
          })
        } else {
          vm.axios.get(`/products?keyword=${vm.$route.query.title}`).then(result => {
            vm.result = result.data.data
          })
        }

      });
    } else {
      next(vm => {
        vm.value = vm.$route.query.s;
        vm.onSearch(vm.$route.query.s)
        vm.axios.get(`/products?keyword=${vm.$route.query.title}`).then(result => {
          vm.result = result.data.data
        })
      });
    }

  },
  beforeRouteLeave (to, from, next) {
    if (to.name != "Detail") {
      this.result = []
      this.payload = ''
      this.value = ''
      this.priceOrder = ''
      this.salesOrder = ''
    }
    next()
  },
  methods: {
    getdata () {
      this.axios.get(`/products?keyword=${this.$route.query.title}&priceOrder=${this.priceOrder}&salesOrder=${this.salesOrder}`).then(result => {
        this.result = result.data.data
      });
    },
    onSearch (val) {
      this.payload = val
      if (this.payload) {
        this.$nextTick(() => {
          this.axios.get(`/products?keyword=${this.payload}`).then(result => {
            this.result = result.data.data
          })
        });
      }

    },
    changenum () {
      this.num++
      if (this.payload) {
        this.$nextTick(() => {
          this.axios.get(`/products?keyword=${this.payload}&priceOrder=${this.priceOrder}&salesOrder=${this.salesOrder}`).then(result => {
            this.result = result.data.data
          });
        });
      } else {
        this.$nextTick(() => {
          this.axios.get(`/products?keyword=${this.$route.query.title}&priceOrder=${this.priceOrder}&salesOrder=${this.salesOrder}`).then(result => {
            this.result = result.data.data
          });
        });
      }

      if (this.num == 1) {
        this.priceOrder = 'asc'
      } else if (this.num == 2) {
        this.priceOrder = 'desc'
      } else if (this.num == 4) {
        this.num = 1
        this.priceOrder = 'asc'
      } else { this.priceOrder = '' }
    },
    changenum2 () {
      this.num2++
      if (this.payload) {
        this.$nextTick(() => {
          this.axios.get(`/products?keyword=${this.payload}&priceOrder=${this.priceOrder}&salesOrder=${this.salesOrder}`).then(result => {
            this.result = result.data.data
          });
        });
      } else {
        this.$nextTick(() => {
          this.axios.get(`/products?keyword=${this.$route.query.title}&priceOrder=${this.priceOrder}&salesOrder=${this.salesOrder}`).then(result => {
            this.result = result.data.data
          });
        });
      }
      if (this.num2 == 1) {
        this.salesOrder = 'asc'
      } else if (this.num2 == 2) {
        this.salesOrder = 'desc'
      } else if (this.num2 == 4) {
        this.num2 = 1
        this.salesOrder = 'asc'
      } else { this.salesOrder = '' }
    },
  },

}
</script>

<style lang="less" scoped>
.goods_list {
  .tabbar {
    display: flex;
    border-bottom: 1px solid #eee;
    div {
      flex: 1;
      line-height: 43px;
      text-align: center;
      font-size: 14px;
      &:first-child {
        &.red {
          color: red;
        }
      }
      &:nth-child(2) {
        align-items: center;
        position: relative;
        i {
          display: block;
          color: #888;
          font-size: 12px;
          &.fa-caret-up {
            position: absolute;
            top: 11px;
            right: 20px;
            &.active {
              color: red;
            }
          }
          &.fa-caret-down {
            position: absolute;
            top: 17px;
            right: 20px;
            &.active {
              color: red;
            }
          }
        }
      }
      &:nth-child(3) {
        align-items: center;
        position: relative;
        i {
          display: block;
          color: #888;
          font-size: 12px;
          &.fa-caret-up {
            position: absolute;
            top: 11px;
            right: 20px;
            &.active {
              color: red;
            }
          }
          &.fa-caret-down {
            position: absolute;
            top: 17px;
            right: 20px;
            &.active {
              color: red;
            }
          }
        }
      }
    }
  }
  .productlist {
    display: flex;
    flex-wrap: wrap;
    padding: 0 10px;
    .pro {
      flex: 0.4;
      height: 259px;
      img {
        display: block;
        width: 172.5px;
        height: 172.5px;
      }
      h2 {
        font-size: 15px;
        text-overflow: ellipsis;
        overflow: hidden;
        white-space: nowrap;
        width: 155.5px;
        padding: 10px 8px;
      }
      .price {
        color: red;
        display: flex;
        align-items: center;
        font-weight: bold;
        padding: 0 8px;
        i {
          font-size: 13px;
        }
        h1 {
          font-size: 17px;
        }
      }
      .vipprice {
        font-size: 12px;
        display: flex;
        padding: 8px;
        justify-content: space-between;
        .left {
          display: flex;
          font-weight: bold;
          img {
            width: 23px;
            height: 10.5px;
          }
        }
        span {
          color: #aaa;
        }
      }
    }
  }
}
</style>